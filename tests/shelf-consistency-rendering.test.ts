...
test("shelf entry API does not create duplicate finished activity for already-finished books", () => {
	...
	const activityStatusBlock = source.slice(
		source.indexOf("status_activity as ("),
		source.indexOf("rating_activity as (")
	);

	assert.match(source, /const shouldRecordStatusActivity = !previousStatus \|\| previousStatus !== status/);
	assert.match(activityStatusBlock, /insert into user_activity/);
	assert.match(activityStatusBlock, /\$\{status\}/);
	assert.match(activityStatusBlock, /where \$\{shouldRecordStatusActivity\}::boolean/);
