...
test("page navigation and route rendering avoid avoidable blocking work", () => {
	...
	assert.match(authorDetail, /getRuntimeCacheValue<Awaited<ReturnType<typeof fetchOpenLibraryBio>>>/);
	assert.match(authorDetail, /warmAuthorExternalCache\(\{\s*key: bioCacheKey/s);
	assert.match(authorDetail, /getRuntimeCacheValue<ExternalAuthorBook\[\]>\(externalBooksCacheKey\)/);
	assert.match(authorDetail, /warmAuthorExternalCache\(\{\s*key: externalBooksCacheKey/s);
	assert.match(authorDetail, /operationName: "external\.open-library"/);
	assert.match(authorDetail, /AbortSignal\.timeout\(900\)/);
	assert.doesNotMatch(authorDetail, /ensureCanonicalWorkSchema\(sql\)/);
	assert.doesNotMatch(authorDetail, /ensureSeriesSchema\(sql\)/);
	assert.match(authorDetail, /timePagePerf\("DogEared Works"/);
	assert.match(authorDetail, /timePagePerf\("Series lookup"/);
	assert.match(authorDetail, /timePagePerf\("ratings aggregation"/);
	assert.match(authorDetail, /timePagePerf\("genre aggregation"/);
	assert.match(authorDetail, /timePagePerf\("shelf-state lookup"/);
	assert.match(authorDetail, /from user_book\s+where book_id = any\(\$\{rawBookIds\}::bigint\[\]\)/);
	assert.match(authorDetail, /from book_genre\s+where book_id = any\(\$\{rawBookIds\}::bigint\[\]\)/);
	assert.match(authorDetail, /select distinct on \(book_id\)/);
