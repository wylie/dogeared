...
test("admin users support search, detail counts, and safe deletion", () => {
	...
	const detail = readFileSync("src/pages/admin/users/[username].astro", "utf8");
	const data = readFileSync("src/lib/adminData.ts", "utf8");
	const adminUserListRows = data.slice(data.indexOf("export async function loadAdminUserListRows"), data.indexOf("export async function loadAdminUsersIndex"));
	assert.equal(list.includes('name="q"'), true);
	assert.equal(list.includes("Pagination"), true);
	assert.equal(list.includes('operationName: "page.admin-users"'), true);
	assert.equal(list.includes("boundedDataQueryCount"), true);
	...
	assert.equal(data.includes("loadAdminUserListRows"), true);
	assert.equal(data.includes("countAdminUsers"), true);
	assert.equal(data.includes("with visible_users as"), true);
	assert.equal(data.includes("join visible_users vu on vu.id = ub.user_id"), true);
	assert.equal(data.includes("limit ${pagination.pageSize}"), true);
	assert.equal(data.includes("offset ${pagination.offset}"), true);
	assert.equal(data.includes("au.email_hash = ${emailHash}"), true);
	assert.equal(data.includes("pgp_sym_decrypt(vu.email_enc"), true);
	assert.equal(adminUserListRows.includes("lower(coalesce(u.email, '')) like"), false);
	assert.equal(data.includes("idx_app_user_created_desc"), true);
	assert.equal(data.includes("idx_user_book_user_updated"), true);
