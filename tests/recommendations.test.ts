...
test("recommendation UI is wired into home, discover, and book detail", () => {
	...
	assert.match(home, /showRecommendationFeedback=\{true\}/);
	assert.match(home, /recommendationFeedbackBookId=\{numericBookId\}/);
	assert.doesNotMatch(home, /data-action="recommendation-feedback"/);
	...
	assert.match(discover, /showRecommendationFeedback=\{true\}/);
