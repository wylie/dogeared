...
Admins can open `/admin/books/[workId]` from Catalog Review Queue or an admin-only Book Detail shortcut to edit canonical Work fields separately from Edition-specific fields. Work edits cover title, canonical title, author, description, subjects, genres, Series, Series position, original publication year, and preferred cover. Edition edits cover format, ISBNs, publisher, publication date/year, page count, cover, provider IDs, language, audiobook duration, location count, chapter count, and an optional Edition title. Audiobook duration is stored as machine-readable seconds in Edition metadata; Kindle/location and chapter counts also live in Edition metadata. Manual edits mark metadata provenance as `Manual`, record manual override fields, and write `admin_catalog_audit_event` rows. The editor shows an impact preview for readers, shelf entries, progress events, journal entries, and Editions before saving; it updates catalog metadata only and preserves reader-owned shelves, progress, journal entries, ratings, reading formats, reading dates, and finish dates.

The Catalog Editor is issue-aware. It loads the same Data Health findings as the Catalog Review Queue, shows a concise summary directly under the title, and links each issue to the relevant field or field group. Critical and needs-attention fields use the DogEared error palette on the actual input/select/textarea, warning icons with screen-reader text, and helper copy explaining why the data matters; healthy fields keep normal input styling. Progress-related fields explain the features they support, such as percentage tracking, Reading Activity, Daily Reading Volume, reading statistics, Series pages, reading order, and Previous/Next navigation. The form is grouped into Work Identity, Series, Classification, Description, Cover, Edition Identity, Publication, Progress Metadata, and External Identifiers using headings, spacing, and subtle dividers inside the Work Metadata and Edition Metadata cards instead of heavy subsection boxes. Cover editing shows a preview and source first, with the raw URL treated as advanced replacement information. As admins fill missing values, the visible warning state and summary update immediately; saving then reloads Data Health so resolved issues disappear from the queue.
