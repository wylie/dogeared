...
DogEared stores first-class series records with name, optional description, optional cover, total-book count, ordered Work entries, publication order, chronological order, and extensible metadata. Book pages show a Series section as a responsive grid of reusable BookCards, led by the series name as the primary heading with "Series" as a small context label. The current Work is highlighted through the card treatment, while missing-title placeholders, links to available Works, and the shared shelf action appear on every entry. Lightweight Previous Book, Jump to Current, and Next Book navigation appears above the grid when those targets exist. Search results, discovery cards, recommendation cards, author cards, and book detail cards display concise series labels where metadata exists.
