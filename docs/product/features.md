...
Profile rendering loads privacy-critical profile data first, then runs independent shelf/activity/goal/achievement/review/navigation reads concurrently where correctness allows. Profile GET requests reuse request-scoped session data, avoid blocking on schema/index setup that is not required to render an already-deployed database, generate favorite book/author links without DB lookups, consolidate follow counts into one query, and bound recent activity/current-reading reads to the visible Profile window. Admin Performance records `page.profile` spans for authentication/session, profile identity, follower/following counts, shelf summary, current reads, momentum/streak, recent activity, finished-this-year books, reading goal, reviews, achievements, Want to Read, Read, DNF, viewer shelf state, custom shelves, favorite links, shared navigation/sidebar data, and notification count.
...
Readers earn persistent achievement badges for meaningful, non-competitive reading milestones. Initial badges cover 7, 14, 30, 60, 100, and 365 day reading streaks plus finishing every currently available book in a series. The registry also includes a future-ready yearly reading goal badge so goal milestones can use the same badge system. Badges appear near the bottom of the main profile header card on the owner profile and, when public achievement visibility is enabled, on public profiles viewed by other readers. The header shows compact colorful icon/name badges that wrap inside the profile card, plus a small more control when additional badges exist. Profile rendering loads earned achievements through the shared registry-backed achievement loader so definition metadata is available before badges render.
...
Reader-facing cover display uses one canonical resolution contract shared by catalog lookups, Book Detail, BookCard-fed list surfaces, and Reading Life: use the selected or reader-associated Edition cover when the page knows an Edition; otherwise use the Work's preferred/default Edition cover where available; then use the Work preferred-cover fallback; then show the standard placeholder. Legacy `book.cover_url` remains a compatibility source for older rows and is treated as Edition-equivalent, not as a separate admin-facing cover model. Rendering must not call Google Books or Open Library just to fill a missing cover; background enrichment may consult providers and persist the result. Cards and Reading Life cover slots include a broken-image fallback so stale remote URLs collapse to the same placeholder instead of leaving a blank image.
...
Search returns canonical books, not provider records. DogEared catalog and matching editorial collection results load first; when external providers are needed, Google Books and Open Library supply evidence that is reconciled into one canonical Work result before it reaches the reader-facing UI. Known DogEared catalog matches reuse existing Work, Edition, author, cover, and series metadata immediately instead of waiting for provider lookup or optional enrichment.

External provider calls run concurrently from normalized query variants and use short-lived public caches keyed by normalized provider, query inputs, and pagination. The Search API reconciles provider results before rendering by grouping first on stable identifiers such as existing DogEared Work, ISBN, Open Library Work/Edition IDs, Google Books IDs, stored source mappings, Edition keys, and canonical Work keys. When identifiers are missing, it uses conservative normalized title and primary-author matching with title-noise guards for summaries, study guides, complete works, graphic adaptations, short ambiguous titles, and incompatible publication years. A bounded, batched canonical Work detector then checks only indexed candidate sets returned by those stable signals and scores only the returned DogEared candidates. If a provider returns a book already in DogEared, Search returns the existing Work with DogEared community context, reader counts, rating context, series metadata, and the standard ShelfButton instead of exposing a duplicate imported record. User-specific shelf state is applied separately from current reader data and is not cached as public search data.

Provider metadata is merged as field-level evidence, not blindly overwritten. Existing DogEared metadata wins for manually curated or trusted catalog fields; provider values can fill missing description, cover, publisher, page count, identifiers, genres, subjects, and publication data when confidence is sufficient. Conflicting Edition-specific facts such as ISBN, publisher, page count, format, cover, or publication date are preserved as Edition variants rather than averaged into one fake value. Search result payloads keep provider source and field-provenance metadata for Catalog Editor, Data Health, enrichment, and debugging, but normal reader cards do not ask readers to choose between Google Books and Open Library.

The Search page appends one reconciled external result set without duplicate provider cards and ignores stale responses when the query changes. It treats loading, visible results, completed zero-results, and error as distinct states: a `/search?q=...` render shows the query and a stable loading/skeleton state until the progressive search lifecycle completes, and "No results" plus "Search again" only appear after all intended sources finish with zero valid results. It propagates request abort signals into provider fetches so abandoned stale searches can stop external work early where supported. Search does not run canonical Work/Edition backfill inside a reader search request; external results attempt canonical matching against existing Work data within a strict timeout and return as unresolved partial matches if the canonical step times out. Search performance telemetry records spans for local catalog search, Google Books, Open Library, provider normalization, metadata merge, canonical Work matching, canonical candidate preparation, identifier matching, ISBN matching, edition lookup, normalized title matching, series matching, existing Work lookup, candidate scoring, result ranking, rendering preparation, timeout counts, retry count, DB query count, candidate comparison count, canonical candidate count, resolved-canonical count, and reconciliation merge counts, which are visible in Admin Performance breakdowns and slow-operation rows. Users can add search results to shelves, catalog matches show series name/book number when available, and matching editorial collections appear above book results.
...
Signed-in readers can open `/reading-life` from the You navigation to see a private reflection on their reading history. The page shows books completed this year, pages read, reading streak, goal progress, average rating, average pages per day, average book length, reading pace, current books, favorite genre, favorite author, and newest author discovered. It also includes a finished-book timeline with year/month/search filters, Reading Activity, genre insights, author insights, fun statistics, and yearly reading-journey summaries.

Reading Activity is the shared daily-reading feature on My Reading Life. Calendar is the consistency and pattern view: it keeps one square per calendar day and uses the same page-equivalent daily volume metric for heat intensity. Volume is the intensity and amount view: it keeps one bar per calendar day for Today, 7 Days, 30 Days, 6 Months, and 365 Days, defaulting to 30 Days. Long ranges remain daily instead of being aggregated into weeks or months; their chart expands horizontally inside a native scroll container with readable bar widths, subtle edge fades, light month markers, and newest-day initial positioning. Both views use the same canonical daily-reading aggregation instead of calculating activity separately.

Daily reading activity exposes date, active state, normalized page-equivalent volume, progressed book count, progress-update count, finish count, per-Work breakdown, and a normalization state. Progress volume is based on recorded deltas rather than absolute progress values. Page progress is exact page-equivalent movement. Percentage, Kindle Location, Chapter, and Audiobook Time updates use the normalized progress event saved by the reading-progress flow; if a total/page equivalent is unavailable, DogEared marks the update incomplete rather than inventing missing totals. Finished books can contribute derived page-equivalent volume only when there is no same-day recorded progress for that Work and a page count exists. Tooltips in both Calendar and Volume show the same underlying daily numbers, including Work breakdowns and any updates that could not be converted.
...
Admins can review operational performance telemetry at `/admin/performance`: request volume, p50/p75/p95/p99 latency, error rate, slow-operation count, core workflow cards, route/API performance, timing-span breakdowns, aggregate span bars, external provider latency, recent slow operations, release comparisons, and workflow-specific performance targets. Recent Slow Operations link to `/admin/performance/operations/[id]`, an admin-only request detail view with a request summary, bottleneck callout, elapsed-time waterfall, nested span breakdown, sanitized diagnostic counts, and a sortable span table.

Recorded operations use stable names such as `search.books`, `progress.save`, `shelf.mutate`, `page.profile`, `page.reading-life`, `page.search`, `page.book-detail`, `page.author-detail`, `page.discover`, `page.admin-users`, `navigation.feedback`, `external.google-books`, and `external.open-library`. Timing spans use the same terminology as local development diagnostics, such as local catalog, external providers, canonical resolution, progress mutation, milestone work, Profile identity, follower/following counts, shelf summary, current reads, recent activity, finished-this-year books, Read, reviews, Admin Users authorization, user-list database loading, shelf/count aggregation, pagination/count queries, shared navigation/sidebar loading, viewer-state loading, and navigation start to skeleton/content swap. New span records can include `startMs` and `parentName` for waterfall/flame rendering; older records that only include duration remain valid and render as non-ordered duration bars.

The telemetry path is fire-and-forget from measured workflows, samples normal successes through `PERFORMANCE_TELEMETRY_SAMPLE_RATE` when configured, and always keeps errors and unusually slow operations. Raw request traces are retained for 45 days; longer-term performance review should rely on aggregate percentile/span metrics and release comparisons rather than permanent raw request logs.
...
Admins can inspect import quality, duplicate risk, potential duplicate Works, duplicate Editions, broken Series, multiple canonical Work assignments, missing-series signals, missing canonical titles, missing series positions, incorrect standalone classification, canonical title conflicts, series position conflicts, recent backfills, canonical title cleanup candidates, and format-aware catalog metadata health. Catalog Review Queue checks Work and Edition records for missing page counts, missing descriptions, audiobook duration, location/chapter counts, covers, publication data, identifiers, Series position, malformed titles, duplicate Work risk, and progress-blocking normalization gaps. The Catalog Review Queue is the primary metadata review surface: it aggregates all current issues into one row per editable review target, uses the highest issue severity as the row severity, and opens one Catalog Editor action for that record. If an Edition exists, Work-scoped issues found while reviewing that Edition are attached to the same Work + Edition queue row and keep their Work scope inside the issue metadata; truly Work-only issues use the Work row. The queue summary cards link directly to filters such as Critical, Missing page counts, Audiobook durations, Progress blocking, and Potential duplicate Works. The queue is server-paginated at 25 unique review targets per page, preserves filter/search/page state in the URL, and uses the shared DogEared pagination control so large health queues stay responsive. Potential Duplicate Works provides a review queue with confidence and reasoning, plus merge and ignore actions. The same duplicate detection can run as the repeatable `cleanup:duplicate-works` task, which defaults to dry-run and only applies merges when explicitly requested. Canonical Work normalization is available through Data Health and `cleanup:canonical-works`; it attaches known-series metadata, repairs Work keys, moves Editions, removes stale placeholders, normalizes redundant title suffixes, and merges only high-confidence duplicate Works. Canonical Title Cleanup reports books whose stored title still includes redundant series parentheticals and provides an admin action to normalize those titles through the same safe metadata-layer matcher used by imports.

Admins can open `/admin/books/[workId]` from Catalog Review Queue or an admin-only Book Detail shortcut to use the unified Book Catalog Editor. The editor is organized around editorial repair rather than database ownership: Editions, Book, Series, Edition & Format, Progress Metadata, Cover, and collapsed Advanced metadata. The Editions area lets admins select which Edition is being edited, add another Edition beneath the same Work, mark the preferred Edition, and attempt deletion only when the Edition has no reader-owned references. Underneath, Work edits still cover title, canonical title when needed, author, description, subjects, genres, Series, Series position, original publication year, preferred cover, and preferred Edition metadata. Edition edits still cover format, ISBNs, publisher, publication date/year, page count, cover, provider IDs, language, audiobook duration, location count, chapter count, and optional Edition title. Audiobook duration is stored as machine-readable seconds in Edition metadata; Kindle/location and chapter counts also live in Edition metadata. Manual edits mark metadata provenance as `Manual`, `Admin upload`, or `Admin URL`, record manual override fields, and write `admin_catalog_audit_event` rows. The unified sticky Save action coordinates Work, Series, preferred Edition, selected Edition, cover, and identifier changes in one transaction, shows an impact preview for readers, shelf entries, progress events, journal entries, and Editions, and preserves reader-owned shelves, progress, journal entries, ratings, reading formats, reading dates, and finish dates.

The Catalog Editor is issue-aware. It loads the same Data Health findings as the Catalog Review Queue, shows a concise summary directly under the title, and links each issue to the relevant field or field group. Critical and needs-attention fields use the DogEared error palette on the actual input/select/textarea, warning icons with screen-reader text, and concise helper copy such as "Page count needed" or "Duration needed"; healthy fields keep normal input styling and concise helper text. Format is controlled as Physical book, Ebook, or Audiobook, and Progress Metadata emphasizes page count for physical/ebook records, duration for audiobooks, and format selection first for unknown data. Series is a searchable combobox: admins can select an existing Series, remove Series membership, or choose an explicit create result for a normalized new Series without leaving the editor; Series position is enabled only when an existing or pending-created Series is selected and accepts a positive whole-number position such as `2`, not `2 of 3`. DogEared derives reader-facing totals such as `Book 2 of 3` from Series membership records. Cover editing targets the selected Edition and shows the cover DogEared currently uses, resolving selected Edition cover first, Work preferred-cover fallback second, and a placeholder last. Removing a selected Edition cover updates the preview immediately and labels any visible fallback as a Work fallback; the remove action is hidden when only a fallback is visible. Admins upload JPEG, PNG, or WebP covers as the primary workflow, can replace the selected Edition cover, can use an Edition URL only from Advanced cover options, and can edit the Work fallback cover URL from Advanced metadata. External identifiers and duplicate-looking fields such as canonical title and Edition title remain editable in Advanced metadata but no longer dominate ordinary cleanup. As admins fill missing values, the visible warning state and summary update immediately; saving then reloads Data Health so resolved issues disappear from the queue and invalidates local catalog/search/home/list runtime caches that could otherwise show a stale cover.

Future provider refreshes should present source-specific suggestions rather than blindly overwrite DogEared data. When Google Books, Open Library, imports, and manual values disagree, the admin should see the source, value, and provider identifier and choose the trusted value. Existing manual override metadata is the protection mechanism that prevents automated enrichment from silently replacing curated title, author, Series, page count, audiobook duration, or ISBN relationships.

Limitations: Most backfill execution lives in scripts. Canonical title cleanup is available as a targeted admin action because it is deterministic and idempotent. Duplicate Work merging is admin-approved rather than automatic because false merges are more damaging than temporary duplicate suggestions. The catalog editor repairs existing Works and Editions; creation of entirely new catalog records still flows through Search, import, or backfill pipelines. The first editor release records the metadata needed for derived-history recalculation, but raw progress history is not rewritten by admin saves; derived Reading Activity views should recompute from source progress events and corrected catalog metadata.

### Admin User Management

Status: Complete

Admins can search users, inspect user counts, view user detail, and delete users except themselves. The `/admin/users` index is a shallow, server-paginated reader-management view: it loads only visible page rows, decrypts email only for those rows, searches exact email matches by `email_hash`, filters username/profile-name matches in SQL, and aggregates shelf/review/last-activity counts only for the visible users. Expensive reader detail data such as streak repair, achievements, full reading statistics, and richer account history stays on `/admin/users/[username]`. Admin Performance records `page.admin-users` spans for authorization, schema/index setup, pagination/count query, bounded user-list query, shelf/count aggregation, deferred reading statistics, deferred streak calculations, deferred achievement loading, profile data projection, and final server rendering.
