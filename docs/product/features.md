...
Book pages show a Reading Journal section for signed-in readers who have that exact book on a shelf. Currently Reading books expose a Write Journal Entry form on the book page, and all owned book pages show recent private entries plus a link to the filtered journal view. After profile reading progress is saved, DogEared offers a compact inline prompt to create a journal entry with the current book and page position. The prompt uses standard DogEared button styling and can be dismissed for that book and position during the current browser session.
