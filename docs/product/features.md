...
The Catalog Editor is issue-aware. It loads the same Data Health findings as the Catalog Metadata queue, shows a concise summary directly under the title, and links each issue to the relevant field or field group. Critical fields use warning styling, warning icons with screen-reader text, and helper copy explaining why the data matters. Progress-related fields explain the features they support, such as percentage tracking, Reading Activity, Daily Reading Volume, reading statistics, Series pages, reading order, and Previous/Next navigation. The form is grouped into Work Identity, Series, Classification, Description, Cover, Edition Identity, Publication, Progress Metadata, and External Identifiers. Cover editing shows a preview and source first, with the raw URL treated as advanced replacement information. As admins fill missing values, the visible warning state and summary update immediately; saving then reloads Data Health so resolved issues disappear from the queue.
