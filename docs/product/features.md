...
Reading Activity is the shared daily-reading feature on My Reading Life. Calendar is the consistency and pattern view: it keeps one square per calendar day and uses the same page-equivalent daily volume metric for heat intensity. Volume is the intensity and amount view: it keeps one bar per day and supports Today, 7 Days, 30 Days, 6 Months, and 365 Days, defaulting to 30 Days. Both views use the same canonical daily-reading aggregation instead of calculating activity separately.
