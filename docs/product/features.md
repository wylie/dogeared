...
The Catalog Editor is issue-aware. It loads the same Data Health findings as the Catalog Review Queue, shows a concise summary directly under the title, and links each issue to the relevant field or field group. Critical and needs-attention fields use the DogEared error palette on the actual input/select/textarea, warning icons with screen-reader text, and concise helper copy such as "Page count needed" or "Duration needed"; healthy fields keep normal input styling. Format is controlled as Physical book, Ebook, or Audiobook, and Progress Metadata emphasizes page count for physical/ebook records, duration for audiobooks, and format selection first for unknown data. Series is a searchable combobox: admins can select an existing Series, remove Series membership, or create a normalized new Series inline without leaving the editor; Series position is enabled only when a Series is selected. Cover editing targets the selected Edition and shows the cover DogEared currently uses, resolving selected Edition cover first, Work preferred-cover fallback second, and a placeholder last. Removing a selected Edition cover updates the preview immediately and labels any visible fallback as a Work fallback. Admins upload JPEG, PNG, or WebP covers as the primary workflow, can remove/replace the cover, and can use a URL only from Advanced cover options. External identifiers and duplicate-looking fields such as canonical title and Edition title remain editable in Advanced metadata but no longer dominate ordinary cleanup. As admins fill missing values, the visible warning state and summary update immediately; saving then reloads Data Health so resolved issues disappear from the queue.
