...
Reading Format records how a reader experienced a specific shelf/progress instance, not what formats a shared Book exists in. The value is stored on `user_book.reading_format`, never on `book`, so the same Work can be read physically once, listened to later, and read as an ebook in another year without changing shared catalog metadata. Supported stable values are `physical`, `ebook`, and `audio`; `unknown` is retained internally for legacy rows and imports that do not contain format information. When a reader moves a book to Currently Reading from the app UI, DogEared asks "How are you reading this book?" with one-click Physical Book, Ebook, and Audiobook choices, defaulting focus to Physical Book. Readers can later change the value from the Profile progress controls without changing progress, journal entries, reading dates, finish date, ratings, or shelves. BookCards keep the standard title and author layout and do not show reading-format icons in the header; the Profile progress controls are the primary display and editing surface for the current reading format. Reading Timeline surfaces may show subtle accessible format icons for known finished formats and omit the icon for Unknown. My Reading Life derives Books, Pages, Reading Days, Finished Books, and percentage breakdowns by format from the reader's own shelf/progress history; timeline filtering supports All, Physical, Ebook, and Audiobook.
