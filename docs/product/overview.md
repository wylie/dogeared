...
When a book belongs to a series, the book page shows a dedicated Series section led by the series name, with "Series" treated as a small context label and the current position as supporting text. Ordered entries render as compact reusable BookCards with covers, title, author, series number, publication metadata, rating context, and the shared shelf action. The current book is subtly highlighted through the card treatment rather than an extra badge. Lightweight Previous Book, Jump to Current, and Next Book navigation appears above the cards; previous and next target adjacent series entries, falling back to Search when a known title is not yet in DogEared, while Jump to Current smooth-scrolls only when the highlighted card is not already fully visible. Book Detail resolves series from `series_book` first and falls back to canonical Work-level `book_work.series_id` metadata so a representative Work does not lose its series context.
