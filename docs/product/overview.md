...
- Reading progress: update pages read for Currently Reading books, mark a book Read, record how the reader is experiencing the book, and record private progress events for metrics without noisy feed posts.
...
Authors have an index page, canonical author detail routes, optional bio/photo/source fields, reader and shelf counts, editorial collection references, and book lists. A legacy author query route redirects to the canonical author route. Author Detail renders DogEared catalog data first from stored author, Work, Edition, Series, rating, genre, and shelf-state rows; it batches per-book Series/ratings/genres instead of resolving those for each card. Optional Open Library bio/photo and external missing-book discovery are cache-only during SSR and are warmed after the local page response when absent.
...
Profile rendering reuses the public profile bundle's already-loaded profile data rather than re-querying the same user row. Profile shelf rendering uses compound indexes for user/status/order lookups, only loads a bounded Want to Read display window for the requested page, keeps recent activity/currently-reading reads bounded to visible Profile windows, and uses shelf counts from the authoritative aggregate summary. Profile GET requests do not wait on schema/index maintenance work that is not required for correctness on an already-deployed database. Favorite book and favorite author links are generated from stored profile text instead of doing render-blocking catalog lookups. After profile privacy is resolved, independent reads for shelf summary, recent activity, currently reading, momentum/streak, achievements, Want to Read, Read, custom shelves, and favorite links run concurrently where safe. The shared layout can reuse the profile route's resolved session for the same request, and Profile performance telemetry is recorded after shared navigation/sidebar data is loaded.

`page.profile` telemetry records sanitized timing spans for authentication/session, profile identity, follower/following counts, shelf summary, currently reading, momentum/streak, recent activity, finished-this-year books, reading goal, reviews, achievements, Want to Read, Read, DNF, viewer shelf state, custom shelves, favorite links, shared navigation/sidebar data, and notification count. Profile spans include relative start offsets where available so Admin Performance can render request waterfalls. DNF is currently zero-work telemetry because it is not an active persisted default shelf in the shelf API; notification count is client-deferred and must not block SSR. These spans are operational diagnostics only and must not include usernames, profile content, book titles, journal text, emails, or request payloads.
...
For existing DogEared Works, default shelf mutations take the local catalog path: they reuse the representative book, Work, and Edition IDs and skip optional metadata/enrichment/canonicalization work before responding. The shelf mutation API combines the previous-state read with the authoritative `user_book` write, batches default/custom shelf reconciliation with activity/progress writes, and builds the response from the persisted row it just saved. Required finished-book milestone checks remain on the response path, while reading-streak checks only run when new forward progress was recorded. Import-style saves without a known `bookId` still go through canonical Work resolution so duplicate prevention and Edition relationships are preserved.
...
Currently Reading Works can store total pages, current page, finished date after completion, and progress events. Edition metadata such as page count or audiobook format may inform calculations, but changing editions must not lose progress. The profile progress control uses a compact grouped selector for Page Number, Percentage, Chapter, Kindle Location, and Audiobook Time plus value, Save, and Finish actions. Percentage input is entered as 0-100 percentage points, so `14` means 14%, not 0.14. All input types normalize into the same canonical persisted field, `currentPage`, before the dedicated progress-save API writes the update. Percentage input converts to `round(totalPages * percent / 100)` when total pages are known; percentage updates without a usable total-page count are rejected instead of silently saving zero progress. The progress-save API is the authoritative normalization boundary because it can load the reader's stored `user_book.total_pages` and fall back to the catalog `book.page_count` for the existing DogEared Work. Profile-side validation may reject clearly malformed values immediately, but it must not reject a numeric percentage only because the client-side card dataset is missing a total that the API can resolve. Non-finish progress saves reuse the existing DogEared `user_book` row and do not run shelf mutation, external metadata lookup, cover enrichment, canonical Work resolution, or broad profile refresh work before updating the UI. The progress-save response returns the saved progress plus authoritative momentum, streak, and guidance data so the card, momentum panel, and journal prompt update without a full page refresh. Forward page progress creates `user_reading_progress_event` rows for streaks, Reading Life, and momentum, but it does not create another Recent Activity shelf event. Read Works can store finished date, rating, and public review metadata.

### Reading Formats

Reading Format records how a reader experienced a specific shelf/progress instance, not what formats a shared Book exists in. The value is stored on `user_book.reading_format`, never on `book`, so the same Work can be read physically once, listened to later, and read as an ebook in another year without changing shared catalog metadata. Supported stable values are `physical`, `ebook`, and `audio`; `unknown` is retained internally for legacy rows and imports that do not contain format information. When a reader moves a book to Currently Reading from the app UI, DogEared asks "How are you reading this book?" with one-click Physical Book, Ebook, and Audiobook choices, defaulting focus to Physical Book. Readers can later change the value from the Profile progress controls without changing progress, journal entries, reading dates, finish date, ratings, or shelves. Profile progress controls display legacy or unset `unknown` values as the disabled placeholder "Choose reading format…" so readers are prompted to choose Physical book, Ebook, or Audiobook instead of treating Unknown as a real format. BookCards keep the standard title and author layout and do not show reading-format icons in the header; the Profile progress controls are the primary display and editing surface for the current reading format. Reading Timeline surfaces may show subtle accessible format icons for known finished formats and omit the icon for Unknown. My Reading Life derives Books, Pages, Reading Days, Finished Books, and percentage breakdowns by format from the reader's own shelf/progress history; timeline filtering supports All, Physical, Ebook, and Audiobook.
...
Reading Activity unifies the historical Reading Calendar and daily reading-volume chart into one section. Calendar answers "When did I read, and how consistently?" with a year-style heatmap. Volume answers "How much did I read on each day?" with one bar per calendar day and range controls for Today, 7 Days, 30 Days, 6 Months, and 365 Days. DogEared preserves daily fidelity for every Volume range; 6 Month and 365 Day views scroll horizontally with readable daily bars, light month markers, and newest days visible first instead of compressing or aggregating days. Both views share the same canonical daily-reading dataset, using recorded progress deltas and page-equivalent normalization rather than separate visual calculations.
...
The Performance Analytics dashboard is operational telemetry rather than product analytics. It records workflow names, route/API identifiers, total duration, success/failure, HTTP status, release version, environment, external provider identifiers, and sanitized timing spans for operations such as `search.books`, `progress.save`, `shelf.mutate`, `page.profile`, `page.book-detail`, `page.author-detail`, `page.discover`, `page.admin-users`, `navigation.feedback`, `external.google-books`, and `external.open-library`. Timing spans may include relative start offsets and parent-span names so admins can inspect individual request waterfalls at `/admin/performance/operations/[id]`; historical duration-only events render as duration breakdowns without fabricated ordering. Admins use `/admin/performance` to inspect p50/p75/p95/p99 latency, error rate, route performance, workflow span breakdowns, aggregate span bars, external provider latency, slow operations, request-level waterfalls, release comparisons, and workflow-specific targets. It must not store search query text, book titles, journal content, email addresses, usernames, profile content, authorization data, or sensitive request payloads.
...
Admins review metadata quality from `/admin/data-health`, including potential duplicate Works, canonical title cleanup, missing covers, authors, descriptions, page counts, audiobook durations, Edition identifiers, provider provenance, progress-blocking metadata gaps, and likely series gaps. Duplicate Work review shows the proposed canonical Work, duplicate Work, confidence, and reasoning, then lets admins merge or ignore each suggestion. Catalog Review Queue is the primary Data Health inbox for Work and Edition health: each editable review target appears once with an aggregated Needs attention list, highest-severity row status, and one Edit Catalog Data action. Work-scoped issues attach to the same Work + Edition target when an Edition row can repair the combined issue set, while truly Work-only issues remain Work rows. Summary cards and quick filters narrow the same queue instead of opening separate gap reports. It is paginated on the server at 25 unique review targets per page and keeps search, filters, and page state in query parameters. It links to `/admin/books/[workId]`, where the unified Book Catalog Editor lets admins repair Book, Series, Edition & Format, Progress Metadata, Cover, and Advanced metadata without choosing between Work and Edition panels. The editor includes an Editions manager for selecting the current Edition, adding another Edition beneath the same Work, marking the preferred Edition, and blocking destructive Edition deletion when reader-owned history is attached. It still saves Work-owned data and Edition-owned data to their correct records, supports inline Series search/creation through explicit select-or-create results, treats Series position as a positive whole-number position while deriving Series totals from membership records, resolves covers as selected Edition cover then Work preferred-cover fallback then placeholder, treats uploads as manually curated provenance, hides remove when only fallback cover is visible, exposes Work fallback cover editing only in Advanced metadata, and makes progress metadata format-aware. Field-level Data Health warnings disappear as soon as current form values are valid, while persisted health issues clear after the unified page-level save. Manual catalog edits preserve reader-owned shelves, progress, journal entries, ratings, reading formats, reading dates, and finish dates, record provenance as manual overrides, and write admin audit events. Reader imports should not be blocked on metadata cleanup.
...
External search is progressive. When provider results are needed, the Search page starts separate Google Books and Open Library external requests concurrently so either provider can append results as soon as it finishes. Provider calls still run from normalized query variants, short-lived public caches avoid duplicate provider work for identical public queries, and external results run through bounded canonical Work detection before they are rendered or saved. Search-time canonical detection is lookup-only: it batches stable identifiers, ISBNs, Edition keys, canonical Work keys, and exact series signals for the current provider response, then scores only the bounded DogEared candidate set in memory. It does not run import-level fuzzy catalog scans, Work creation, Edition creation, enrichment, or canonical backfill. Provider caches and resolved external-result caches are keyed by normalized provider, query input, and pagination only; reader-specific shelf state is not cached as shared public data and is applied from the current reader's shelf data when cards render. Slow provider calls are bounded so one dependency cannot hold the entire progressive search open indefinitely.

The Search page renders DogEared results first, then appends non-duplicate external matches as they arrive. Client-side search requests use abort/request identifiers so older responses cannot replace newer queries, while the existing inline spinner, visible query text, and right-aligned clear control remain stable. Search distinguishes loading, visible results, completed zero-results, and error states; "No results" is only shown after all intended sources finish with zero valid deduped matches. Search also propagates request abort signals into provider fetches so abandoned stale searches can stop external work early. Search does not run canonical Work/Edition backfill on the request path. External results attempt canonical matching against existing Work data within a strict budget; if a provider or canonical match times out, Search returns partial results and records timeout metadata for Admin Performance instead of holding the request open.
...
Internal navigation remains server-rendered by default and is progressively enhanced through Astro's client router on the shared layout. Same-origin links and GET forms can swap without a full document reload while direct URLs, deep links, browser history, and accessibility semantics continue to work as ordinary SSR pages. The persistent shell shows a delayed top progress indicator and route-shaped skeleton overlay for slow swaps so readers get immediate destination feedback without a full-screen spinner or layout shift. Skeleton patterns exist for Search, Profile, My Reading Life, Book Detail, Author Detail, Discover/Books, Following, Notifications, Journal, and generic pages; they set `aria-busy`, use a polite live region, and honor reduced-motion preferences. The shared layout may reuse request-scoped session data and short-lived public layout data, but user-specific navigation state such as unread notification counts stays per reader and is refreshed from authenticated endpoints rather than globally cached. Admin Performance records sanitized `navigation.feedback` events for navigation start to skeleton visible and navigation start to content swap, using route patterns rather than query strings or usernames.

Representative page routes avoid blocking render on repeated setup or optional public metadata work. Reading Journal, Notifications, My Reading Life, Following, Book Detail, and Profile memoize idempotent schema-readiness checks per server process and reset on failure; Author Detail assumes the deployed schema for read-only rendering and leaves schema maintenance to migrations and mutating/admin paths. Independent page reads are parallelized where correctness allows. Book Detail preserves canonical Work resolution and duplicate prevention while avoiding redundant Work lookups and moving optional known-series maintenance off the response critical path. Author Detail renders local DogEared author and book data first, batches Series/ratings/genres/shelf-state lookups, and treats optional Open Library biography and missing-book lookups as cache-only SSR metadata with background warming. These public external caches are keyed by normalized author inputs and local book IDs and never include reader shelf state.
