...
export const PATCH: APIRoute = async ({ request }) => {
	try {
		...
		if (!session?.userId) return json(401, { error: "You must be logged in to update shelf order." });
		...
		const order = rawOrder
			...
			.filter(Boolean)
			.slice(0, 64);
		if (order.length === 0) return json(400, { error: "Shelf order is required." });
		...
		return json(500, {
			error: "Failed to update shelf order.",
